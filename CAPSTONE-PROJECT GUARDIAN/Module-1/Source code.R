...
#
# MASTER SHINY DESKTOP APPLICATION (ALL MODULES 1 - 5 INTEGRATED)
#
# STRUCTURE:
...
#   MODULE 2 : Preprocessing & Modelling -> Unlocks when dataset generated.
#   MODULE 3 : Hybrid ABM + DES Simulation -> Q-Learning RL Dispatch Agent.
#   MODULE 4 : Evaluation Metrics & Statistical Testing -> Paired t-Test,
#              One-Sample t-Test, ANOVA across severity, Cohen's d.
#   MODULE 5 : Model Comparison & Visualizations -> 3-Model Head-to-Head,
#              Line, Bar, Grouped Bar, & Multi-Metric System Profile.
#
# Required packages:
...
## Load Modules 3, 4, and 5 as Shiny modules.
source("../Module-3/module3_simulation.R")
...
## collapses a tiny-but-nonzero p-value into a flat "0.00000".
## ---------------------------------------------------------------------
...
ui <- fluidPage(
  ...
  titlePanel(
    div(
      h2("Project Guardian - Intelligent Disaster Response & Resource Coordination"),
      h4("Reinforcement Learning & Hybrid Agent-Based Simulation System")
    )
  ),
  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      tabsetPanel(
        ...
        tabPanel("Module 3: Simulation",
                 br(),
                 mod3_simulation_ui("sim3")),
        
        tabPanel("Module 4: Evaluation Metrics",
                 br(),
                 mod4_evaluation_ui("eval4")),
        
        tabPanel("Module 5: Model Comparison",
                 br(),
                 mod5_comparison_ui("comp5"))
      )
    )
  )
)
...
server <- function(input, output, session) {
  ...
  ## Connect Module 3, Module 4, and Module 5
  sim_results <- mod3_simulation_server("sim3")
    ...
    if (is.null(raw_data())) {
      ...
      tagList(
        ...
        p("Existing System = a simple baseline regression using only two basic resource-count
          predictors. Proposed System = a regression using the full normalized feature set."),
        plotOutput("model_compare_bar", height = "420px"),
        
        hr(),
        h4("Overfitting / Underfitting Diagnostic (Learning Curve)"),
        p("Polynomial complexity vs generalization diagnostic."),
  ...
  ## Module 2 Regression comparison
  ...
  output$overfit_summary <- renderText({
    ...
    paste0(
      ...
      ", Test RMSE = ", round(cc$Test_RMSE[cc$Degree == min(cc$Degree)], 4), " -> UNDERFITTING.\n",
      "- At degree ", max(cc$Degree), " (most complex model): Train RMSE = ", round(cc$Train_RMSE[cc$Degree == max(cc$Degree)], 4),
      ", Test RMSE = ", round(cc$Test_RMSE[cc$Degree == max(cc$Degree)], 4), " -> OVERFITTING."
    )
  })
